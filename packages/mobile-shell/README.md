...
Existing Web Shell file inputs open Android's document picker for single or multiple files (up to 100). Only readable `content://` selections from external providers are returned; app-private paths, ungranted URIs and mixed unsafe results are rejected. No broad storage/media permission or persistent URI grant is requested. Cancel, navigation, switching connections or destroying the Activity cancels the pending callback; an old picker result cannot complete a newer request. MIME filters are hints; the H5 retains its own upload limits, capability and workspace checks. Camera capture and directory selection are not implemented. See the [file-selection design](../../docs/design/mobile-file-selection.md).
